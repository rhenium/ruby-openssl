...
class OpenSSL::TestPKey < OpenSSL::PKeyTestCase
  def test_generic_oid_inspect_rsa
    ...
    assert_match %r{(oid=rsaEncryption|type=RSA)}, rsa.inspect
  end
  ...
  def test_x25519
    ...
    assert_match %r{(oid|type)=X25519}, alice.inspect
  ...
  def test_ml_dsa
    # AWS-LC also supports ML-DSA, but it implements it in a different way
    return unless openssl?(3, 5, 0)

    pkey = OpenSSL::PKey.generate_key("ML-DSA-44")
    assert_match(/type=ML-DSA-44/, pkey.inspect)
