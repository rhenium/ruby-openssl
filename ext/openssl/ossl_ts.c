...
ossl_tsreq_set_nonce(VALUE self, VALUE num)
{
    ...
    ret = TS_REQ_set_nonce(req, nonce);
    ASN1_INTEGER_free(nonce);
    if (!ret)
	ossl_raise(eTimestampError, "TS_REQ_set_nonce");
    return num;
}
...
ossl_tsreq_to_der(VALUE self)
{
    ...
    GetTS_REQ(self, req);
    mi = TS_REQ_get_msg_imprint(req);
    if (!(TS_MSG_IMPRINT_get_algo(mi) && TS_MSG_IMPRINT_get_msg(mi))) {
...
ossl_ts_verify(int argc, VALUE *argv, VALUE self)
{
    ...
    if (!TS_VERIFY_CTX_set_store(ctx, store)) {
	TS_VERIFY_CTX_free(ctx);
	ossl_raise(eTimestampError, NULL);
    }
    ...
    if (!TS_VERIFY_CTS_set_certs(ctx, certs))
	goto end;
...
ossl_tsfac_create_ts(VALUE self, VALUE key, VALUE certificate, VALUE request)
{
    ...
    TS_RESP_CTX_set_time_cb(ctx, ossl_tsfac_time_cb, (void *)gen_time);
