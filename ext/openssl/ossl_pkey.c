 ...
 *
 * Hashes and signs the _data_ using a message digest algorithm _digest_ and
 * a private key _pkey_.
 *
 ...
 *
 *   # Creates a copy of the RSA key _pkey_, but without the private components
 *   pub_key = pkey.public_key
 *   puts pub_key.verify("SHA256", signature, data, signopts) # => true
 *
 * _options_ parameter was added in version 3.0.
 ...
 *
 * Verifies the _signature_ for the _data_ using a message digest algorithm
 * _digest_ and a public key _pkey_.
 *
 ...
 *
 * Signs _data_ using a private key _pkey_. Unlike #sign, _data_ will not be
 * hashed by _digest_ automatically.
 *
 ...
 *   A String that represents the message digest algorithm name, or +nil+
 *   if the PKey type requires no digest algorithm.
 *   Although this method will not hash _data_ with it, this parameter may still
 *   be required.
 * data::
 ...
 *
 * Verifies the _signature_ for the _data_ using a public key _pkey_. Unlike
 * #verify, this method will not hash _data_ with _digest_ automatically.
 *
 * Returns +true+ if the signature is successfully verified, +false+ otherwise.
 * The caller must check the return value.
 *
 ...
 *
 * Recovers the signed data from _signature_ using a public key _pkey_.
 *
 * signature::
 *   A String containing the signature to be verified.
 *
 ...
 *
 * Performs a public key encryption operation using _pkey_.
 *
 ...
 * options::
 *   A Hash that contains algorithm-specific control strings.
 *   See the man page EVP_PKEY_CTX_ctrl_str(3) for details.
 *
 * Example:
 *   pkey = OpenSSL::PKey.generate_key("RSA", rsa_keygen_bits: 2048)
 *   data = "secret data"
 ...
 *
 * Performs a public key decryption operation using _pkey_.
 *
